/**
 * Deploy the Level 1 private counter contract to a Midnight network.
 *
...
const counterWitnesses = {
  privateIncrement(context: { privateState: { privateIncrement: bigint } }) {
    return [context.privateState, context.privateState.privateIncrement] as const;
  },
};

const compiledContract = CompiledContract.make('private-counter', Counter.Contract).pipe(
  (CompiledContract.withWitnesses as any)(counterWitnesses),
...
async function main() {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      // Midnight.js 4.1.x supplies private witness state via privateStateId +
      // initialPrivateState. The secret value stays in the local private-state
      // provider and is consumed by the privateIncrement witness.
      deployed = await deployContract(providers, {
        compiledContract: compiledContract as any,
        args: [],
        privateStateId: PRIVATE_STATE_ID,
        initialPrivateState: { privateIncrement: PRIVATE_INCREMENT },
      });
      break;
    } catch (err: any) {
  ...
  console.log('  Private counter is live on Preview.\n');
}
